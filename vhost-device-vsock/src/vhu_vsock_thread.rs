            ...
            match polled {
                Ok(()) => {
                    let ready: Vec<(RawFd, bool)> = events
                        ...
                        .map(|event| {
                            (
                                event.token().0.try_into().expect("invalid fd"),
                                event.is_writable(),
                            )
                        })
                        .collect();
...
mod tests {
    ...
    fn test_vsock_thread(backend_info: BackendType) {
        ...
        VhostUserVsockThread::poll_register(&t.poller, dummy_fd.as_raw_fd(), Interest::WRITABLE)
            .unwrap();
    ...
    fn test_vsock_thread_failures() {
        ...
        assert!(
            VhostUserVsockThread::poll_register(&invalid_poller, -1, Interest::READABLE).is_err()
        );
        assert!(
            VhostUserVsockThread::poll_modify(&invalid_poller, -1, Interest::READABLE).is_err()
        );
