...
impl VhostUserVsockThread {
    ...
    pub fn new(
        ...
        let poller = Arc::new(Mutex::new(Poll::new().map_err(Error::PollFdCreate)?));
        ...
        for host_raw_fd in thread.host_listeners_map.keys() {
            VhostUserVsockThread::poll_register(&thread.poller, *host_raw_fd, Interest::READABLE)?;
        }
    ...
    pub fn poll_register(poller: &Arc<Mutex<Poll>>, fd: RawFd, interests: Interest) -> Result<()> {
    ...
    pub fn poll_unregister(poller: &Arc<Mutex<Poll>>, fd: RawFd) -> Result<()> {
        if fd < 0 {
    ...
    pub fn poll_modify(poller: &Arc<Mutex<Poll>>, fd: RawFd, interests: Interest) -> Result<()> {
        let mut source = SourceFd(&fd);

        poller.lock().unwrap()
            .registry()
            .reregister(&mut source, Token(fd as usize), interests)
            .map_err(Error::PollModify)?;
    ...
    pub fn register_listeners(&mut self, poll_handler: Arc<VringEpollHandler<ArcVhostBknd>>) {
        poll_handler
            .register_listener(self.get_poll_fd(), EventSet::READABLE, u64::from(BACKEND_EVENT))
            .unwrap();
        poll_handler
            .register_listener(
            ...
            match polled {
                Ok(()) => {
                    let ready: Vec<(RawFd, bool)> = events
                        .iter()
                        .map(|event| (event.token().0 as RawFd, event.is_writable()))
                        .collect();
    ...
    fn handle_event(&mut self, fd: RawFd, writable: bool) {
        if let Some(listener) = self.host_listeners_map.get(&fd) {
                    ...
                    if self.mem.is_some() {
                        match conn {
                            Ok((stream, addr)) => {
                                ...
                                if let Err(err) = Self::poll_register(
                                    &self.poller,
                                    stream_raw_fd,
                ...
                self.thread_backend.listener_map.entry(fd)
            {
                    ...
                    match conn.tx_buf.flush_to(&mut conn.stream) {
                        Ok(cnt) => {
                            if cnt > 0 {
                                ...
                                if Self::poll_modify(&self.poller, fd, Interest::READABLE).is_err() {
                ...
                // Unregister the stream from the poller and register it again when the connection is
                // established with the guest
                let _ = Self::poll_unregister(&self.poller, fd);
    ...
    fn add_stream_listener(&mut self, stream: UnixStream) -> Result<()> {
        ...
        VhostUserVsockThread::poll_register(&self.poller, stream_fd, Interest::READABLE)?;
...
mod tests {
    ...
    fn test_vsock_thread(backend_info: BackendType) {
        ...
        .unwrap();
        VhostUserVsockThread::poll_modify(&t.poller, dummy_fd.as_raw_fd(), Interest::READABLE)
            .unwrap();
        VhostUserVsockThread::poll_unregister(&t.poller, dummy_fd.as_raw_fd()).unwrap();
    ...
    fn test_vsock_thread_failures() {
        ...
        assert!(VhostUserVsockThread::poll_register(&invalid_poller, -1, Interest::READABLE).is_err());
        assert!(VhostUserVsockThread::poll_modify(&invalid_poller, -1, Interest::READABLE).is_err());
        assert!(VhostUserVsockThread::poll_unregister(&invalid_poller, -1).is_err());
